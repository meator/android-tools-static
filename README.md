# WORK IN PROGRESS
This branch is currently under heavy development. Once finished, it will be
incorporated into the `master` branch.

_No_ guarantees are made about the commit linearity of this branch (I reserve
the right to force push to this branch).

# android-tools-static
android-tools-static is a fork of [android-tools](upstream) which adds support for fullstatic compilation using [musl libc](musl) on Linux. It adapts MSYS2's patches to make this project buildable on Windows without extra effort.

This repository provides convenient prebuilt release artifacts for personal use or for inclusion in other software. Unlike Google's official release archives, the release artifacts, like the repository itself, are licensed under Apache 2.0 [(more info)](license_considerations.md).

android-tools-static makes an effort to support cross compilation well.
...
Go to the [Releases](https://github.com/meator/android-tools-static/releases) section, download and extract the latest `TODO` archive and build the project as follows:
...
1. `standardlayout`
   ...
   **All executables do not have a suffix (even on Windows!)** This is done to make the layout more standardized.
...
Here is a list of trusted GitHub Actions which are trusted (excluding those from https://github.com/actions):

1. https://github.com/msys2/setup-msys2 - provided offitialy by MSYS2
...
3. https://github.com/softprops/action-gh-release - used by https://github.com/meator/AdbWinApi, the [official GitHub Action which handles making releases](https://github.com/actions/create-release) was deprecated, this action is one of its one of the alternatives mentioned in its README
