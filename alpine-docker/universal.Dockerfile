...
RUN apk add --no-cache make patch gcc g++ file perl python3 build-base linux-headers autoconf automake libtool bison rsync
...
ADD config-base.mak ${TARGET_MAK_FILE} .
...
RUN <<EOF
set -e
apk del --rdepends make patch gcc g++ file perl python3 build-base linux-headers autoconf automake libtool bison rsync
rm -rf ${MUSL_CROSS_MAKE_VERSION}.tar.gz config-versions.mak config-base.mak save-versions.py ${TARGET_MAK_FILE}
EOF
